...
echo "사용할 녹음 장치는 몇번 장치입니까? (0번부터 시작)"
read device
while true
do
	echo "$device 번 장치가 맞습니까?[Y/N]"
	read ans
	if [ "$ans" = "Y" ];
	then
		break
	else
		echo "사용활 녹음 장치는 몇번 장치입니까? (0번 부터시작)"
		read device
	fi
done
...
echo startup application을 설정합니다.
